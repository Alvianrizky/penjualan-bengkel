				...
				<div id="table-data">
					<div class="box-body">
						<div class="form-group">
							<div class="row">
								<div class="col-lg-6">
									<button id="create" class="btn btn-primary btn-sm" title="Data Create" alt="Data Create"><i class="glyphicon glyphicon-plus"></i> Tambah Jasa Service</button>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col-lg-6">
								...
								<div class="form-group">
									...
									<div id="NamaService"></div>
								</div>
								<div class="form-group">
									<label>Biaya Service</label>
									<div id="BiayaService"></div>
								</div>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	var site_url = site_url() + 'jasaservice/';
	...
	$(document).ready(function() {
		...
		$('#create').click(function() {
			$.post(site_url + 'form_data/', function(data) {
				form_data();
				$('.box-title').text('Tambah Jasa Service');

				data = JSON.parse(data);
				$('#hidden').html(data.hidden);
				$('#NamaService').html(data.NamaService);
				$('#BiayaService').html(data.BiayaService);
			});
		});

		$('#submit').click(function() {
			$.post(site_url + 'save_jasaservice/', $('#form-data').serialize(), function(data) {
					if (data.code == 1) {
	...
	function table_data() {
		...
		$('.box-title').text('Jasa Service List');
	}
	...
	function form_view() {
		$('p#hidden').empty();
		$('p#NamaService').empty();
		$('p#BiayaService').empty();

		$('#table-data').hide();
		$('#form-data').hide();
		$('#form-view').show();

		$('.box-title').text('View Jasa Service');
	}
	...
	function view_data(id) {
		$.post(site_url + 'view/', {
				...
				$('p#NamaService').html(data.NamaService);
				$('p#BiayaService').html(data.BiayaService);
			})
			.fail(function(jqXHR, textStatus, errorThrown) {
				alert('Error adding / update data');
			});
